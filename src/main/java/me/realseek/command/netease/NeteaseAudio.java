    ...
    public void onCommand(User sender, Object[] arguments, @Nullable Message message) {
        if (InChannel.inChannel(sender, arguments, message)) {
            try {
                ...
                // 赋值
                netease.setName(Main.getNetease().getName());
                netease.setArtName(Main.getNetease().getArtName());
                netease.setMusicPicUrl(Main.getNetease().getMusicPicUrl());
                netease.setMuiscId(musicId);
                netease.setSender(sender);
