    ...
    public void onCommand(User sender, Object[] arguments, @Nullable Message message) {
        if (InChannel.inChannel(sender,arguments,message)) {
            try {
                ...
                // 赋值
                bilibili.setBvNum(Main.getBilibili().getBvNum());
                bilibili.setpNum(Main.getBilibili().getpNum());
                bilibili.setPic(Main.getBilibili().getPic());
                bilibili.setTitle(Main.getBilibili().getTitle());
                bilibili.setCid(Main.getBilibili().getCid());
                bilibili.setUpName(Main.getBilibili().getUpName());
                bilibili.setAudioUrl(musicDownloadUrl);
                bilibili.setSender(sender);
