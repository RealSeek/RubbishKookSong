    ...
    public void onCommand(User sender, Object[] arguments, @Nullable Message message) {
        if (InChannel.inChannel(sender, arguments, message)) {
            try {
                ...
                // 赋值
                qqMusic.setName(Main.getQqMusic().getName());
                qqMusic.setSongmid(Main.getQqMusic().getSongmid());
                qqMusic.setPicUrl(Main.getQqMusic().getPicUrl());
                qqMusic.setArtName(Main.getQqMusic().getArtName());
                qqMusic.setSender(sender);
